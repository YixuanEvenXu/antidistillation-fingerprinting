...
# ----------------------------------------------------------------------------
# Stage 6 – Student lm evaluation harness runs
# ----------------------------------------------------------------------------
stage6_args=(
    --batch_size "${LM_EVAL_BATCH_SIZE}"
    --log_samples
    --model_args "pretrained=${LORA_DIR},dtype=${STUDENT_DTYPE}"
    --tasks "${LM_EVAL_TASKS}"
    --output_path "${LM_EVAL_DIR}"
    --wandb_args name=${MODEL_IDENTIFIER},project=${LM_EVAL_PROJECT}
)
if [[ -n "${LM_EVAL_EXTRA_ARGS}" ]]; then
    stage6_args+=(${LM_EVAL_EXTRA_ARGS})
fi

run_stage "Stage 6 – Student LM Evaluation Harness" "$SENTINELS_DIR/stage6_lm_eval_${student_tag}_${method_label}_lr${LR_TAG}_e${EPOCHS}.done" "${FORCE_STAGE6_LM_EVAL:-0}" \
    "lm-eval" "run" "${stage6_args[@]}"
